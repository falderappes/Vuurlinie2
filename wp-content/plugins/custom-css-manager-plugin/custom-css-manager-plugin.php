...
class Custom_CSS_Manager_Plugin{
	...
	public function __construct() {
		...
		add_action( 'wp_head', array(&$this, 'insert_custom_css_code') );
		add_action( 'login_head', array(&$this, 'insert_custom_css_code') );
		add_action( 'admin_head', array(&$this, 'insert_custom_css_code') );
		
		add_action( 'init', array(&$this, 'display_external_stylesheet'));
...
/**
	...
	**/
	...
	function insert_custom_css_code(){
		$custom_css_code = $this->opt['css_settings']['css_code'];
			
		if (!empty($custom_css_code)) {
			echo "<link rel='stylesheet' type='text/css' media='all' href='". site_url() ."?action=display_custom_css_code' >";
		}
	}
	...
	function display_external_stylesheet(){
		
		$custom_css_code = $this->opt['css_settings']['css_code'];
		
		if( isset($_GET) && isset($_GET['action']) && $_GET['action'] == "display_custom_css_code" ){
			header("Content-type: text/css");
			echo stripslashes($custom_css_code);
			die();
		}
   	...
   	public function admin_menu() {
        ...
        $this->page_menu = add_theme_page( $this->plugin_title, $this->plugin_title, 'manage_options',  $this->setting_name, array($this, 'plugin_settings_page') );
	...
	public function add_plugin_actions($links) {
		array_unshift($links, '<a href="themes.php?page=' . $this->setting_name . '">' . __('Settings') . '</a>');
